...
class InformationRetrieval:
    ...
    def _require_gensim(self):
        if KeyedVectors is None or Word2Vec is None:
            raise ImportError(
                "sentence_wise retriever requires gensim. Install it with "
                "'pip install gensim' before using -retriever sentence_wise."
            )
    ...
    def _train_embeddings(self, docs):
        ...
        self.model = Word2Vec(sentences=sentences,vector_size=self.vector_size, window=self.window,min_count=self.min_count,workers=self.workers,)
